	...
	if (SName.value.length >= 2 && SName.value.length <= 32)
		if (Name.value.length >= 2 && Name.value.length <= 32)
			if (PName.value.length >= 0 && PName.value.length <= 32) {
				...
				showPopup('<div style="padding: 0 48px;">Какие-то данные введены не корректно</div>', null);
		else
			showPopup('<div style="padding: 0 48px;">Какие-то данные введены не корректно</div>', null);
	else
		showPopup('<div style="padding: 0 48px;">Какие-то данные введены не корректно</div>', null);
...
function checkInput() {
	var loginInput = document.getElementById('newlogin');
	var passwordInput = document.getElementById('newpassword');

	var regex = /^[a-zA-Z0-9]+$/;

	if (!regex.test(loginInput.value)) {
		showPopup('Логин может содержать только символы a-z и 0-9', null);
		loginInput.value = '';
	}

	if (!regex.test(passwordInput.value)) {
		showPopup('Пароль может содержать только символы a-z и 0-9', null);
		passwordInput.value = '';
	}
}
